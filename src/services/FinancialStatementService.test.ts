// Tests for FinancialStatementService and InMemoryLedgerRepository
import {
...
describe('InMemoryLedgerRepository', () => {
  ...
  it('findAllAccounts returns all accounts when no org filter', async () => {
    ...
    await repo.saveAccount(makeAccountInput({ code: '2100', type: 'LIABILITY', subType: 'CURRENT_LIABILITY' }));
...
describe('FinancialStatementService', () => {
  ...
  describe('generateProfitAndLoss', () => {
    async function setupAccounts() {
      ...
      const revenue = await service.createAccount(
        makeAccountInput({ code: '4000', name: 'Sales Revenue', type: 'REVENUE', subType: 'REVENUE' }),
      );
      const cogs = await service.createAccount(
        makeAccountInput({ code: '5000', name: 'Cost of Sales', type: 'EXPENSE', subType: 'COST_OF_SALES' }),
      );
    ...
    it('generates a P&L with correct gross profit and net profit', async () => {
      ...
      // Revenue entry: Dr Cash 10,000 / Cr Revenue 10,000
      ...
      // COGS entry: Dr COGS 4,000 / Cr Cash 4,000
      await service.postJournalEntry(
        makeJournalEntryInput(
          [
      ...
      // Admin expense: Dr Admin 1,500 / Cr Cash 1,500
      await service.postJournalEntry(
        makeJournalEntryInput(
          [
      ...
      // Tax: Dr Tax 1,000 / Cr Cash 1,000
      await service.postJournalEntry(
        makeJournalEntryInput(
          [
  ...
  describe('generateBalanceSheet', () => {
    ...
    it('generates a balanced Balance Sheet', async () => {
      ...
      // Issue share capital: Dr Cash 50,000 / Cr Share Capital 50,000
      await service.postJournalEntry(
        makeJournalEntryInput(
          [
      ...
      // Buy equipment on credit: Dr PPE 20,000 / Cr LT Debt 20,000
      await service.postJournalEntry(
        makeJournalEntryInput(
          [
      ...
      // Buy goods on credit: Dr Cash 5,000 / Cr Payables 5,000 (advance received)
      await service.postJournalEntry(
        makeJournalEntryInput(
          [
            { accountId: cash.id, direction: 'DEBIT', amount: 5000 },
      ...
      expect(bs.totalAssets).toBe(75000); // cash 55k + ppe 20k
      expect(bs.totalLiabilities).toBe(25000); // payables 5k + ltDebt 20k
  ...
  describe('listJournalEntries', () => {
    it('returns only posted entries', async () => {
      ...
      // Force a draft entry directly via the repository
      await repo.saveJournalEntry(
        makeJournalEntryInput(
          [
